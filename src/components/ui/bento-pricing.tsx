        ...
        <div ref={containerRef} className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-8">
            {pricingData.map((plan) => {
                ...
                if (isGrowth) {
                    return (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 p-4">
                                <Badge variant="secondary">{plan.titleBadge}</Badge>
                                {plan.isRecommended && (
                ...
                return (
                    <PricingCard
                        key={plan.id}
                        titleBadge={plan.titleBadge}
                        priceLabel={plan.priceLabel}
                        priceSuffix={plan.priceSuffix}
                        features={plan.features}
                        className={cn(
                            plan.id === 'basic' && "lg:col-span-3",
                            plan.id === 'professional' && "lg:col-span-4",
                            plan.id === 'enterprise' && "lg:col-span-4"
                        )}
                    />
                );
            })}
